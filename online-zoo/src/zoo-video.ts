...
const API_KEY: string = 'AIzaSyBL4VuxnvuESkpjHHwySxaWtUrb9Y9fOnQ'; 

async function loadPandaLive(): Promise<void> {
    const videoContainer = document.getElementById('panda-video-container');

    // ვამოწმებთ, არის თუ არა ეს კონტეინერი ამ გვერდზე
    if (!videoContainer) return;

    const query: string = "giant+panda+live+cam+wolong+grove";
    const url: string = `https://www.googleapis.com/youtube/v3/search?part=snippet&eventType=live&q=${query}&type=video&key=${API_KEY}`;
    ...
    try {
        ...
        if (data.items && data.items.length > 0) {
            ...
            //  iframe-ით კონტეინერის შინგით რაცაა ეგ იცვლება რააა  ამ შემთხვევაშ ვიდეო მომაქ 
