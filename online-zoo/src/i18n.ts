...
export const initI18n = () => {
    ...
    const currentFlag = document.getElementById('current-flag') as HTMLImageElement | null;
    ...
    const updateText = (lang: Language) => {
        ...
        elements.forEach(el => {
            ...
            if (key && translations[lang] && (translations[lang] as any)[key]) {
                el.textContent = (translations[lang] as any)[key];
            }
        });
    };
    ...
    options.forEach(option => {
        option.addEventListener('click', (e) => {
            ...
            const lang = option.getAttribute('data-i18n-lang') as Language;
            ...
            if (lang) {
                ...
                if (newImg && currentFlag) currentFlag.src = newImg;
    ...
    const savedLang = (localStorage.getItem('userLang') as Language) || 'en';
    updateText(savedLang);
    
    if (savedLang === 'de' && currentFlag) {
        currentFlag.src = "https://flagcdn.com/w20/de.png";
        if (currentText) currentText.textContent = "DE";
    }
};